...
#    data_transformer = HAL9001DataTransformer()
    ...
    gbm_model = lgb.LGBMRegressor(n_estimators=100,
                            num_leaves=40,
                            learning_rate=0.1,
                            max_depth = -1,
                            ...
                            subsample = 0.8,
                            random_state=random_seed)
    bagging_gbm = BaggingRegressor(base_estimator=gbm_model, n_estimators=num_bagging_estimators, random_state=random_seed, warm_start=False)

    # SVM Regressor
    svr_model = SVR(degree = 3,
                    C = 3.43, # def 1
                    gamma = 'scale', # def scale
                    kernel = 'rbf', # def rbf
                    shrinking = True)
    bagging_svr = BaggingRegressor(base_estimator=svr_model, n_estimators=num_svr_bagging_estimators, random_state=random_seed, warm_start=False)

    # KNeighbors Regressor
    #knn_model = KNeighborsRegressor(n_neighbors = 12, #  default=5
    #                    weights = 'distance',# distance, default=uniform
    #                    algorithm = 'auto', # ball_tree, kd_tree, brute, default=auto
    #                    leaf_size = 30, #default=30
    #                    p = 2 #default=2
    #                    )
    #bagging_knn = BaggingRegressor(base_estimator=knn_model, n_estimators=num_bagging_estimators, random_state=random_seed, warm_start=False)


    # Adaboost
    #adabr_model = AdaBoostRegressor()
    #bagging_adabr = BaggingRegressor(base_estimator=adabr_model, n_estimators=num_bagging_estimators, random_state=random_seed, warm_start=False)
    ...
    if predict_on_test:
        ...
        test_name = 'V42-VotBag10GbrXgbLgbm+SrvBag20-OutlrIQR-Ovr200KLikeRmoved-RanSta42-CoxBoxY-gitvers-xxxx-2020-12-14'
