...
#    print('Repeated K-Fold CV RMSLE: %.10f (%.5f)' % (np.mean(rep_scores), np.std(rep_scores)))
    ...
    if predict_on_test:
        ...
        test_tosubmit_folder = os.path.join(log_folder,'V32-VotBag10GbrXgbLgbm-OutlierRemSVM+PowerTrans')
        # Create log folder if does not exist
        if not Path(test_tosubmit_folder).exists():
            os.mkdir(test_tosubmit_folder)
        test_name = 'V32-VotBag10GbrXgbLgbm-OutlierRemSVM+PowerTrans-RanSta42-CoxBoxY-gitvers-xxxx-2020-12-12'
